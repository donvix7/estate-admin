...
const DeleteEstateModal = ({ estateName, id }) => {
...
const router = useRouter();
  ...
  const handleDelete = () => {
    ...
    setTimeout(() => {
      ...
      setIsDeleting(false);
      setIsOpen(false);
      alert(`${estateName} has been purged from the system.`);
      router.push("/dashboard/estates")
      // In a real app, use router.push('/dashboard/estates') here
    }, 1500);
  };
  ...
  return (
    <>
      ...
      <button 
        ...
        className="px-6 py-2.5 rounded-md text-red-600 dark:text-red-400 font-bold text-xs uppercase tracking-widest hover:bg-red-50 dark:hover:bg-red-400/5 transition-colors cursor-pointer font-manrope"
      >
        Delete Estate
      </button>
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl shadow-2xl relative z-10 overflow-hidden border border-slate-100 dark:border-slate-800 animate-in zoom-in-95 slide-in-from-bottom-8 duration-500">
            ...
            <div className="bg-red-50 dark:bg-red-950/20 p-8 border-b border-red-100 dark:border-red-900/30">
              ...
              <p className="text-sm font-body text-slate-500 dark:text-slate-400 leading-relaxed">
                You are about to purge <span className="font-bold text-slate-900 dark:text-white">{estateName}</span> ({id}). 
                This action will delete all resident history, financial blueprints, and architectural documentation.
              </p>
            </div>
...
export default DeleteEstateModal;
